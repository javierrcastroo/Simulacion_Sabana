/*
 * sheet.c
 * Simulation of normal vibration modes of an ideal elastic membrane.
 *
 * Model:  d²u/dt² = c²∇²u
 *
 * Boundary conditions:
...
/* ------------------------------------------------------------------ */
#define N        4000      /* grid nodes per side                      */
#define T_SIM    25.0      /* total simulation time [s]                */
...
int main(void)
{
    ...
    const double dt = 0.5 * h / c;          /* CFL: r = 0.5           */
    ...
    const double omega = (c * M_PI / L) * 0.02;
    ...
    for (int k = 1; k <= nstep; k++) {
        ...
        for (int j = 1; j <= N - 2; j++) {
            for (int i = 0; i < N; i++) {
                int il = (i == 0)     ? 1     : i - 1;  /* ghost left  */
                int ir = (i == N - 1) ? N - 2 : i + 1;  /* ghost right */
                double lap = u_curr[ir][j] + u_curr[il][j]
                           + u_curr[i][j+1] + u_curr[i][j-1]
                           - 4.0 * u_curr[i][j];
                u_next[i][j] = 2.0 * u_curr[i][j] - u_prev[i][j] + r2 * lap;
            }
        }
